...
if(missing(geft) || missing(lb) || missing(leg) || missing(col) || 
missing(ylim) || missing(maintitle )) 
...
plot(0, 0, xlim=c(0, L), ylim=ylim, xlab=xlab, ylab=ylab, type="n", 
main=maintitle, xaxt="n", cex.main=0.8)
...
basistype=smoothdata$fd$fd$basis[[2]], ftest=ftest, nperm=nperm, 
lb=lb, mul=1.5, xat=xat)
